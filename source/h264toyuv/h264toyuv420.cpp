...
int main(int argc, char* argv[])
{
	...
	av_image_fill_arrays(pFrameYUV420->data, pFrameYUV420->linesize,
	...
	while (av_read_frame(AFCtx_p, packet) >= 0)
	{
		if (packet->stream_index == videoindex)
		{
			...
			sws_scale(img_convert_ctx,
				...
				0, ACCtx_p->height, pFrameYUV420->data, pFrameYUV420->linesize);

			y_size = ACCtx_p->width * ACCtx_p->height;
			fwrite(pFrameYUV420->data[0], 1, y_size, fp_yuv);//Y
			fwrite(pFrameYUV420->data[1], 1, y_size / 4, fp_yuv);//U
			fwrite(pFrameYUV420->data[2], 1, y_size / 4, fp_yuv);//V
		}
	}
	fclose(fp_yuv);
	av_free(buf);
