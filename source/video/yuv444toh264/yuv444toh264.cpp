...
int main(int argc, char* argv[])
{
	...
	char filename_in[] = "e:/ffmpeg/720yuv444cuda.yuv";
#if TEST_HEVC
	...
	char filename_out[] = "e:/ffmpeg/yuv-444.h264";
#endif
	...
	for (i = 0; i < framenum; i) {
		...
		if (fread(pFrame->data[0], 1, y_size, fp_in) <= 0 ||		// Y
			fread(pFrame->data[1], 1, y_size, fp_in) <= 0 ||	// U
			fread(pFrame->data[2], 1, y_size, fp_in) <= 0) {	// V
			return -1;
		}
		else if (feof(fp_in)) {
			break;
