...
int main(int argc, char* argv[])
{
	...
	img_convert_ctx = sws_getContext(ACCtx_p->width, ACCtx_p->height,
		ACCtx_p->pix_fmt,
		ACCtx_p->width, ACCtx_p->height, AV_PIX_FMT_RGB24, SWS_BICUBIC, NULL, NULL, NULL);
	if (!img_convert_ctx)
	{
	...
	//获取转换后YUV数据的大小
	int video_size = av_image_get_buffer_size(AV_PIX_FMT_RGB24, ACCtx_p->width,
		ACCtx_p->height, 1);
	...
	while (av_read_frame(AFCtx_p, packet) >= 0)
	{
		if (packet->stream_index == videoindex)
		{
			...
			sws_scale(img_convert_ctx,
				...
				0, ACCtx_p->height, pFramergb->data, pFramergb->linesize);

			y_size = ACCtx_p->width * ACCtx_p->height;
			fwrite(pFramergb->data[0], 1, y_size * 3, fp_rgb);
			//fwrite(pFramergb->data[0], 1, y_size, fp_rgb);//Y
			//fwrite(pFramergb->data[1], 1, y_size/4, fp_rgb);//U
			//fwrite(pFramergb->data[2], 1, y_size/4, fp_rgb);//V
		}
	}
	fclose(fp_rgb);
	av_free(buf);
