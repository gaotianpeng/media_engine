...
int AudioEncoder::InitAAC(int channels, int sample_rate, int bit_rate) {
    ...
    if (!codec) {
        LOG(ERROR) << "avcodec_find_encoder AV_CODEC_ID_AAC failed\n";
        return -1;
    }
    codec_ctx_ = avcodec_alloc_context3(codec);
    if (!codec_ctx_) {
        LOG(ERROR) << "avcodec_alloc_context3 AV_CODEC_ID_AAD failed\n";
        return -1;
    }
    ...
    if (ret < 0) {
        LOG(ERROR) << "avcodec_open2 failed" << std::endl;
        return -1;
    }

    LOG(INFO) << "InitAAC success\n";
    return 0;
}
...
int AudioEncoder::Encode(AVFrame *frame, int stream_index, int64_t pts, int64_t time_base,
    ...
    while(1)
    {
        ...
        if (ret == AVERROR(EAGAIN) || ret == AVERROR_EOF) {
            ...
            LOG(ERROR) << "aac avcodec_receive_packet failed\n";
            av_packet_free(&packet);
            ret = -1;
        }
